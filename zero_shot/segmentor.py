...
# import clip
from models.OpenaiMaskCLIP.clip import load, tokenize
from models.OpenaiMaskCLIP.clip_reg import ModifiedImageEncoder
from pamr import PAMR
from prompts.imagenet_template import openai_imagenet_template


@MODELS.register_module()
class DistilledMaskCLIP(BaseSegmentor):
    def __init__(self, clip_path, distilled_weights_path, name_path, device=torch.device('cuda'),
                 patch_size=16, pamr_steps=10, pamr_stride=(8, 16), prob_thd=0.0, logit_scale=40, 
                 slide_stride=112, slide_crop=224):

        data_preprocessor = SegDataPreProcessor(mean=[122.771, 116.746, 104.094], std=[68.501, 66.632, 70.323], rgb_to_bgr=True)
        super().__init__(data_preprocessor=data_preprocessor)
        # Load CLIP, but we only use Text Transformer
        self.net, _ = load(clip_path, device=None, jit=False)
        self.net = self.net.to(torch.float32).to(device)
        ...
        # self.num_classes = max(self.query_idx) + 1 # not used
        self.query_idx = torch.Tensor(self.query_idx).to(torch.int64).to(device)

        query_features = list()
        with torch.no_grad():
            for qw in query_words:
                query = tokenize([temp(qw) for temp in openai_imagenet_template]).to(device)
        ...
        # Load distilled student MaskCLIP
        self.regViT = ModifiedImageEncoder(
            self.net.visual,
            num_register_tokens=16,
            patch_size=16,
            input_resolutions=(448,448),
            upsampled_pos_grad=False
        ).to(device)

        self.regViT = self.regViT.to(torch.float32)
        self.regViT.load_state_dict(torch.load(distilled_weights_path))
    ...
    def forward_feature(self, img, logit_size=None):
        ...
        image_features = self.regViT(img)
        image_features /= image_features.norm(dim=-1, keepdim=True)
        image_features = image_features[:,1:-16,:]

        logits = image_features @ self.query_features.T

        # patch_size = self.net.visual.patch_size
        
        w, h = img[0].shape[-2] // self.patch_size, img[0].shape[-1] // self.patch_size
