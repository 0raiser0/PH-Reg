import models.OpenaiMaskCLIP.model
...
def upsampling_pos_embedding(old_patch_pos_embed, patch_size, input_resolution):
    ...
    row_patches = int(math.sqrt(num_patches))
    assert row_patches **2 == num_patches
    # Reshape to (1, 14, 14, 768) => permute => interpolate => (1, 28, 28, 768)
    patch_pos_embed_2d = patch_pos_embed.reshape(-1, row_patches, row_patches, hidden_size).permute(0, 3, 1, 2)
...
class ModifiedResBlock(torch.nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.ln_1 = models.OpenaiMaskCLIP.model.LayerNorm(d_model)
        self.mlp = nn.Sequential(OrderedDict([
            ("c_fc", nn.Linear(d_model, d_model * 4)),
            ("gelu", models.OpenaiMaskCLIP.model.QuickGELU()),
            ("c_proj", nn.Linear(d_model * 4, d_model))
        ]))
        self.ln_2 = models.OpenaiMaskCLIP.model.LayerNorm(d_model)
...
class ModifiedImageEncoder(nn.Module):
    def __init__(self, original_image_encoder, num_register_tokens=2, patch_size=16, input_resolution=224):
        super().__init__()
        ...
        # Either keep or upsample the original patch positional embeddings (but do not expand)
        old_patch_pos_embed = self.original_image_encoder.positional_embedding
        if input_resolution == 224:
            new_patch_pos_embed = old_patch_pos_embed
        else:
            new_patch_pos_embed = upsampling_pos_embedding(
                old_patch_pos_embed,
                patch_size=patch_size,
                input_resolution=input_resolution
            )

        self.original_image_encoder.positional_embedding = nn.Parameter(new_patch_pos_embed.clone())
    ...
    def forward(self, x):
        ...
        old_pos_embed = torch.clone(self.original_image_encoder.positional_embedding.detach()).to(x.device, x.dtype)
        new_pos_embed = eval_pos_embedding(
            old_patch_pos_embed=old_pos_embed,
            patch_size=16,
            input_resolution=(img_h, img_w)
        )
        # x = x + pos_embed
        x = x + new_pos_embed
...
def eval_pos_embedding(old_patch_pos_embed, patch_size, input_resolution):
    ...
    patch_pos_embed_v2 = F.interpolate(
        patch_pos_embed_2d,
        size=(row_resolution // patch_size, col_resolution // patch_size),
        mode='bicubic',
        align_corners=False,
        recompute_scale_factor=False
    )
...
def student_model_initialization(pretrained, num_register_tokens, patch_size, input_resolution):
    # Load the CLIP model
...
#     device = "cuda" if torch.cuda.is_available() else "cpu"
#     model, _ = student_model_initialization(
#         num_register_tokens=4, 
#         input_resolution=224*2, 
#         device=device
#     )
    
#     last_resblock = model.visual.original_image_encoder.transformer.resblocks[-1]
    
#     # Get the source code for the class
#     class_source = inspect.getsource(last_resblock.__class__)
#     print(class_source)   
    
#     # Check the requires_grad status for all parameters
#     for name, param in model.named_parameters():
#         if param.requires_grad:
#             print(f"{name}: requires_grad={param.requires_grad}")
    
#     model = model.to(device)
#     images = torch.randn(2, 3, 224*2, 224*2).to(device)
    
#     result = model.visual(images)
#     print(result.shape)