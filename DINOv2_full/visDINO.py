...
from models.studentDINOv2 import RegDINOv2_Base_ValueOutput
...
def vis_augmented_teacher():
    ...
    parser.add_argument("--distilled_path", type=str, default="/data/chenyinjie/CYJcode/traindistill/DINOv2/distilledweights/distilled_dinov2_weights_40.pth")
    ...
    dinov2 = AutoModel.from_pretrained(
        pretrained_model_name_or_path=args_dict['pretrained_path'],
    ...
    for original_images, shifted_images, shifted_idxs in train_dataloader:
        ...
        with torch.inference_mode():
            ...
            img_feats = img_feats['last_hidden_state']
    ...
    for i in range(args_dict['batch_size']):
        ...
        feature_map = img_feats[i].cpu().float().detach().numpy() # [num_patches, hidden_size]
        eps = 1e-8
    ...
    plt.savefig("orig_DINOv2.png", dpi=300)  # Save as a single image
