...
def vis_augmented_teacher():
    ...
    # parser.add_argument("--save_dir", type=str, default="/data/chenyinjie/CYJcode/distillation/DistillNaCLIP3/artifacts/bf16", help="Directory to save model checkpoints.")
    # training settting
    ...
    parser.add_argument("--counts", type=int, default=14, help="Number of sample points for the shifting augmentaion in Teacher Model.")
    ...
    save_path = '/data/chenyinjie/CYJcode/traindistill/DINOv2_full/'
    # if len(original_images.shape) == 4:
    #     raw_img = original_images[0]
        
    visualize_both_feat_umap(
        raw_feats=raw_teach_feats[0],
        denoised_feats=teacher_img_feats[0],
        m=0.4,
        mx=0.7,
...
def visualize_both_feat_umap(raw_feats, denoised_feats, m, mx, patch_shape=(28, 28), resolution=(448, 448), n_components=3, save_path=None):
    """
    参数说明:
    ...
    """
    # Step 1: UMAP 降维
    # raw_feats = raw_feats / raw_feats.norm(dim=-1, keepdim=True)
    raw_feats_np = raw_feats.detach().cpu().numpy()
    
    # denoised_feats = denoised_feats / denoised_feats.norm(dim=-1, keepdim=True)
    ...
    plt.tight_layout(pad=0)
    if save_path:
        img_path = save_path + '_denoised.svg'
    else:
        img_path = save_path
    # plt.savefig(img_path, format='svg')
    plt.savefig(
