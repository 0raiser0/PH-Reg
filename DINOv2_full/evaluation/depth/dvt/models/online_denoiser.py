    ...
    def forward(
        ...
        if self.vit is not None:
            with torch.no_grad():
                ...
                # fixed
                # print(vit_outputs[0][0].shape)
                # print(vit_outputs[0][1].shape)
                # class_tokens = vit_outputs[0][1]
                # original_feats = vit_outputs[0][0].permute(0, 2, 3, 1)
                # x = original_feats
                ...
                # b, _, c = vit_outputs[0].shape
                original_feats = vit_outputs[0].permute(0, 2, 3, 1)
                # original_feats = vit_outputs[0][:,1:,:].reshape(b, 32, 32, 768)
                x = original_feats
        else:
        ...
        # class_tokens = class_tokens @ self.vit.model.proj
        # x = x @ self.vit.model.proj
