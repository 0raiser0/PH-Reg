...
from utils.denoising import shuffle_shift
...
class VisionTransformer(nn.Module):
    ...
    def forward_images(self, x: torch.Tensor, return_all=False):
        B, nc, w, h = x.shape
    ...
    def forward(self, args_dict, shifted_images, shifted_idxs):
        ...
        teacher_img_feats = self.forward_images(
            x=shifted_images,
            return_all=True
        )
        # Check for NaN/inf in teacher features
        if torch.isnan(teacher_img_feats).any() or torch.isinf(teacher_img_feats).any():
            print(f"[ERROR] Teacher features contain NaN or Inf. Skipping batch.")
        ...
        return out_img_feats

    def interpolate_pos_encoding(self, x, w, h):
        # if len(self.positional_embedding.shape) == 3:
        #     self.positional_embedding = self.positional_embedding.squeeze(0)
